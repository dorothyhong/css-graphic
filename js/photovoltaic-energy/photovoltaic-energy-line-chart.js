(function () {
  ...
  const container = document.getElementById("photovoltaic-energy-line-chart");
  ...
  // Append SVG object
  const svg = d3
    .select("#photovoltaic-energy-line-chart")
    .append("svg")
  ...
  d3.csv("./data/photovoltaic-energy/photovoltaic-energy1.csv").then((data) => {
    ...
    y.domain([0, Math.ceil(d3.max(data, (d) => d["LCOE"]) / 100) * 100]);
